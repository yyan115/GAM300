...
return Component {
    ...
    fields = {
        ...
        Delay = 0.0,           -- Delay in seconds before slash starts
        AttackState = "light_1"
    },
    ...
    Start = function(self)
        ...
        if event_bus then
            local attackState = self.AttackState:gsub('"', '')  -- Remove quote characters
            event_bus.subscribe("attack_performed", function(data)
                if data.state == attackState then
    ...
    Update = function(self, dt)
        ...
        if self.delaying then
            ...
            if self.delayTimer >= self._currentDelay then
                -- Delay finished, start the slash
                self.delaying = false
                self.active = true
                self.age = 0
            ...
            return  -- Don't process slash animation during delay
        end
    ...
    TriggerSlash = function(self, startRot, endRot, speed, delay)
        ...
        self._currentDelay = delay or self.Delay or 0.0  -- delay in seconds
