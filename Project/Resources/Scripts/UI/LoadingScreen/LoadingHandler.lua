...
return Component {
    ...
    Start = function(self)
        ...
        if self._barEntity then
            ...
            if self._barTransform then
                ...
                -- Force start exactly in the middle by shifting right
                local fullX = (self._barBaseScaleX or 1.0) * (self.fullScaleX or 1.0)
                self._barTransform.localPosition.x = self._barBasePosX + (fullX * 0.5)
    ...
    _applyBarProgress = function(self, p)
        ...
        if self._barBasePosX then
            -- Shift position RIGHT by half the missing width to keep it perfectly centered
