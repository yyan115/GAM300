...
std::string Material::CompileToResource(const std::string& assetPath) {
...
bool Material::LoadResource(const std::string& assetPath) {
	std::filesystem::path assetPathFS(assetPath);
	assetPathFS = ResolveToProjectRoot(assetPathFS);

	std::string resourcePath = (assetPathFS.parent_path() / assetPathFS.stem()).generic_string() + ".mat";

	std::cout << "[Material] LOAD - Input path: " << assetPath << std::endl;
	std::cout << "[Material] LOAD - Computed path: " << resourcePath << std::endl;
	std::cout << "[Material] LOAD - Working directory: " << std::filesystem::current_path() << std::endl;

	std::ifstream materialFile(resourcePath, std::ios::binary);
	if (materialFile.is_open()) {
		// Read material name
		size_t nameLength;
	...
	std::cerr << "[Material] Failed to open material file: " << resourcePath << std::endl;
	return false;
}

bool Material::ReloadResource(const std::string& assetPath) {
	return LoadResource(assetPath);
}

std::shared_ptr<AssetMeta> Material::ExtendMetaFile(const std::string& assetPath, std::shared_ptr<AssetMeta> currentMetaData) {
