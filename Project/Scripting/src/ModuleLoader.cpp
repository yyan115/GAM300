...
void ModuleLoader::Initialize(IScriptFileSystem* fs) {
    std::lock_guard<std::mutex> lk(m_mutex);
    m_fs = fs;
    // sensible defaults if none added by caller
...
void ModuleLoader::InstallLuaSearcher(lua_State* L, int pos) {
    ...
    if (insertPos == -1) {
        ...
        lua_rawseti(L, -2, len + 1); // pops our function
    }
    else {
...
bool ModuleLoader::ReloadModule(lua_State* L, const std::string& modulename) {
    ...
    if (lua_pcall(L, 1, 1, 0) != LUA_OK) {
        ...
        if (err) {
            ENGINE_PRINT(EngineLogging::LogLevel::Error, "ModuleLoader::ReloadModule require failed: %s", err);
        }
        lua_pop(L, 1);
        return false;
    }
