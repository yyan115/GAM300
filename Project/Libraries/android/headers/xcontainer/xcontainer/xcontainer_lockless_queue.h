...
#ifdef ANDROID
// Typical ARM cache line size is 64 bytes
                alignas(64) std::atomic<std::size_t> m_Turn;
#else
...
#else
            alignas(xcore::target::getCacheLineSize()) std::atomic<std::size_t> m_Head{ 0 };
#endif
#ifdef ANDROID
            // Typical ARM cache line size is 64 bytes
            alignas(64) std::atomic<std::size_t> m_Tail{ 0 };
#else
            alignas(xcore::target::getCacheLineSize()) std::atomic<std::size_t> m_Tail{ 0 };
#endif
