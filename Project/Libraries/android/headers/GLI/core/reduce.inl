...
namespace detail
{
	...
	struct compute_sampler_reduce_1d
	{
		...
		static vec<4, val_type> call(texture1d const& A, texture1d const& B, binary_func<val_type> TexelFunc, binary_func<val_type> ReduceFunc)
		{
			...
			vec<4, val_type> Result(TexelFunc(SamplerA.template fetch<val_type>(TexelIndex, 0), SamplerB.template fetch<val_type>(TexelIndex, 0)));
			...
			for(size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex)
			{
				...
				for(TexelIndex.x = 0; TexelIndex.x < TexelCount.x; ++TexelIndex.x)
				{
					Result = ReduceFunc(Result, TexelFunc(
						SamplerA.template fetch<val_type>(TexelIndex, LevelIndex),
						SamplerB.template fetch<val_type>(TexelIndex, LevelIndex)));
				}
			}
	...
	struct compute_sampler_reduce_1d_array
	{
		...
		static vec<4, val_type> call(texture1d_array const& A, texture1d_array const& B, binary_func<val_type> TexelFunc, binary_func<val_type> ReduceFunc)
		{
			...
			extent_type TexelIndex(0);
			vec<4, val_type> Result(TexelFunc(SamplerA.template fetch<val_type>(TexelIndex, 0, 0), SamplerB.template fetch<val_type>(TexelIndex, 0, 0)));
			...
			for(size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex)
			{
				...
				{
					Result = ReduceFunc(Result, TexelFunc(
						SamplerA.template fetch<val_type>(TexelIndex, LayerIndex, LevelIndex),
						SamplerB.template fetch<val_type>(TexelIndex, LayerIndex, LevelIndex)));
				}
			}
	...
	struct compute_sampler_reduce_cube
	{
		...
		static vec<4, val_type> call(texture_cube const& A, texture_cube const& B, binary_func<val_type> TexelFunc, binary_func<val_type> ReduceFunc)
		{
			...
			for(size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex)
			{
				...
				{
					Result = ReduceFunc(Result, TexelFunc(
						SamplerA.template fetch<val_type>(TexelIndex, FaceIndex, LevelIndex),
						SamplerB.template fetch<val_type>(TexelIndex, FaceIndex, LevelIndex)));
				}
			}
	...
	struct compute_sampler_reduce_cube_array
	{
		...
		static vec<4, val_type> call(texture_cube_array const& A, texture_cube_array const& B, binary_func<val_type> TexelFunc, binary_func<val_type> ReduceFunc)
		{
			...
			extent_type TexelIndex(0);
			vec<4, val_type> Result(TexelFunc(SamplerA.template fetch<val_type>(TexelIndex, 0, 0, 0), SamplerB.template fetch<val_type>(TexelIndex, 0, 0, 0)));
			...
			for(size_type LevelIndex = 0, LevelCount = A.levels(); LevelIndex < LevelCount; ++LevelIndex)
			{
				...
				{
					Result = ReduceFunc(Result, TexelFunc(
						SamplerA.template fetch<val_type>(TexelIndex, LayerIndex, FaceIndex, LevelIndex),
						SamplerB.template fetch<val_type>(TexelIndex, LayerIndex, FaceIndex, LevelIndex)));
				}
			}
