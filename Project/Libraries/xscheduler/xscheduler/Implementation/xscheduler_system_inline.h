...
namespace xscheduler
{
    ...
    {
    #ifdef _WIN32
        // Windows API accepts wide strings
        SetThreadDescription(GetCurrentThread(), pName);

    #elif defined(__ANDROID__) || defined(__linux__)
        // Convert wchar_t* to UTF-8 narrow string
        std::wstring wstr(pName);
        std::string name_utf8;

        // Minimal UTF-32 → UTF-8 conversion (Android wchar_t is 32-bit)
        for (wchar_t wc : wstr) {
            if (wc < 0x80) {
                name_utf8.push_back(static_cast<char>(wc));
            } else if (wc < 0x800) {
                name_utf8.push_back(static_cast<char>(0xC0 | ((wc >> 6) & 0x1F)));
                name_utf8.push_back(static_cast<char>(0x80 | (wc & 0x3F)));
            } else if (wc < 0x10000) {
                name_utf8.push_back(static_cast<char>(0xE0 | ((wc >> 12) & 0x0F)));
                name_utf8.push_back(static_cast<char>(0x80 | ((wc >> 6) & 0x3F)));
                name_utf8.push_back(static_cast<char>(0x80 | (wc & 0x3F)));
            } else {
                name_utf8.push_back(static_cast<char>(0xF0 | ((wc >> 18) & 0x07)));
                name_utf8.push_back(static_cast<char>(0x80 | ((wc >> 12) & 0x3F)));
                name_utf8.push_back(static_cast<char>(0x80 | ((wc >> 6) & 0x3F)));
                name_utf8.push_back(static_cast<char>(0x80 | (wc & 0x3F)));
            }
        }

        pthread_setname_np(pthread_self(), name_utf8.c_str());

    #elif defined(__APPLE__)
        // macOS version takes const char*
        std::string name_utf8(pName, pName + wcslen(pName));
        pthread_setname_np(name_utf8.c_str());
    #endif
    }
